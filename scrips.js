...
if (sendToWhatsAppButton) {
    sendToWhatsAppButton.addEventListener('click', function (event) {
        ...
        const whatsappMessage = `Name: ${name}\nEmail: ${email}\nProject: ${project}\nMessage: ${message}`; // Added backticks for template literals

        // Encode the message for the URL
        const encodedMessage = encodeURIComponent(whatsappMessage);

        // Construct the WhatsApp link with the message
        const whatsappURL = `https://wa.me/254710678753?text=${encodedMessage}`; // Added backticks for template literals
